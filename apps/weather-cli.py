...
def get_aqi_info(aqi_index):
    aqi_map = {
        1: (Colors.GREEN, "Good"), 2: (Colors.YELLOW, "Fair"),
        3: (Colors.ORANGE, "Moderate"), 4: (Colors.RED, "Poor"),
        5: (Colors.PURPLE, "Very Poor"),
    }
    return aqi_map.get(aqi_index, (Colors.RESET, "Unknown"))

WEATHER_ICONS = {
    "01d": "🌞", "01n": "🌙", "02d": "🌤️", "02n": "🌤️", "03d": "☁️", "03n": "☁️",
...
def get_clothing_suggestions(current_weather, units):
    ...
    suggestions = {'Top': 'T-Shirt', 'Bottom': 'Shorts', 'Outerwear': 'None',
                   'Footwear': 'Sandals', 'Accessory': 'Sunglasses'}

    if 65 <= feels_like_f < 75: suggestions.update({'Bottom': 'Jeans / Trousers', 'Footwear': 'Sneakers'})
    elif 55 <= feels_like_f < 65: suggestions.update({'Top': 'Long-sleeve Shirt', 'Bottom': 'Jeans', 'Footwear': 'Sneakers'})
    elif 45 <= feels_like_f < 55: suggestions.update({'Top': 'Sweater', 'Bottom': 'Jeans', 'Outerwear': 'Light Jacket', 'Footwear': 'Shoes / Boots'})
    elif 32 <= feels_like_f < 45: suggestions.update({'Top': 'Sweater / Fleece', 'Bottom': 'Trousers', 'Outerwear': 'Warm Jacket', 'Footwear': 'Boots', 'Accessory': 'Beanie'})
    elif feels_like_f < 32: suggestions.update({'Top': 'Thermal Layer', 'Bottom': 'Insulated Trousers', 'Outerwear': 'Heavy Winter Coat', 'Footwear': 'Insulated Boots', 'Accessory': 'Beanie & Gloves'})

    if 'rain' in description or 'drizzle' in description:
        if suggestions['Outerwear'] == 'None': suggestions['Outerwear'] = 'Waterproof Jacket'
        if 'Boots' not in suggestions['Footwear']: suggestions['Footwear'] = 'Waterproof Shoes'
        suggestions['Accessory'] = 'Umbrella'
    elif 'snow' in description:
        suggestions['Outerwear'] = 'Waterproof Winter Coat'; suggestions['Footwear'] = 'Waterproof Boots'
...
def read_config():
    config_path = os.path.join(os.path.expanduser("~"), ".config", "weather", "config.ini")
    if not os.path.exists(config_path):
        print(f"Error: Config file not found at {config_path}", file=sys.stderr); sys.exit(1)
    ...
    try:
        ...
        print(f"Error: Missing config in {config_path}: {e}", file=sys.stderr); sys.exit(1)

def get_api_data(endpoint, config, cache):
    """Generic function to fetch data, using cache if valid."""
    cache_key = endpoint.split('/')[0]
    
    if cache_key in cache and time.time() - cache.get(cache_key, {}).get('timestamp', 0) < CACHE_DURATION_SECONDS:
...
def display_current_weather(data, units):
    ...
    print(f" {icon} {description} | {temp_color}{temp:.1f}{temp_unit}{Colors.RESET} (Feels like: {feels_like:.1f}{temp_unit})")
...
def display_clothing_suggestions(suggestions):
    ...
    for i in range(half):
        ...
        part2 = ""
        if i + half < len(items):
            part2 = f"{Colors.CYAN}{items[i+half][0]}:{Colors.RESET} {items[i+half][1]}"
        print(part1 + part2)
...
def display_daily_forecast(data, units):
    ...
    for day, values in list(daily_data.items())[1:6]:
        ...
        print(f" {day_name}: {icon}  Low: {low_color}{low:.0f}{temp_unit}{Colors.RESET} | High: {high_color}{high:.0f}{temp_unit}{Colors.RESET}")

# --- Main Execution ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A command-line weather dashboard using free OWM APIs.")
