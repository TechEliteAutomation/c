...
class PiperTTS(TTSEngine):
    ...
    def is_available(self) -> bool:
        if not self.executable_path.is_file():
            print(
                f"Warning: Piper executable not found at '{self.executable_path}'",
                file=sys.stderr
            )
            return False
        if not self.model_path.is_file():
            print(
                f"Warning: Piper voice model not found at '{self.model_path}'",
                file=sys.stderr
            )
            return False
        ...
        if self._paplay_available is None:
            try:
                ...
                subprocess.run(
                    ['which', 'paplay'], capture_output=True, check=True, text=True
                )
                self._paplay_available = True
            except (subprocess.CalledProcessError, FileNotFoundError):
                print(
                    "Warning: `paplay` command not found. Piper TTS cannot play audio.",
                    file=sys.stderr
                )
    ...
    def speak(self, text: str):
        if not self.is_available():
            print(
                "Error: Piper TTS is not available or configured correctly.",
                file=sys.stderr
            )
            return
        ...
        command_str = (
            ...
            f"{shlex.quote(str(self.executable_path))} --model "
            f"{shlex.quote(str(self.model_path))} --output_file - | "
        ...
        try:
            ...
            print(
                f"Error during Piper TTS execution: {e}",
                file=sys.stderr
            )
            print(
                f"Piper Stderr: {e.stderr.strip()}",
                file=sys.stderr
            )
...
class ESpeakTTS(TTSEngine):
    ...
    def is_available(self) -> bool:
        if self._espeak_available is None:
            try:
                subprocess.run(
                    ['which', 'espeak-ng'], capture_output=True, check=True
                )
                self._espeak_available = True
            except (subprocess.CalledProcessError, FileNotFoundError):
                print(
                    "Warning: 'espeak-ng' command not found. "
                    "eSpeak TTS is unavailable.",
                    file=sys.stderr
                )
    ...
    def speak(self, text: str):
        if not self.is_available():
            print(
                "Error: eSpeak-NG is not available.",
                file=sys.stderr
            )
            return
        ...
        command = [
            'espeak-ng', '-v', self.voice, '-s', self.speed, '-p', self.pitch,
            cleaned_text
        ]
        try:
            subprocess.run(
            ...
            print(
                f"Error during eSpeak-NG execution: {e}",
                file=sys.stderr
            )
            print(
                f"eSpeak-NG Stderr: {e.stderr.strip()}",
                file=sys.stderr
            )
...
def create_tts_engine() -> TTSEngine | None:
    ...
    try:
        ...
        if engine_name == "piper":
            ...
            print(
                f"Warning: Unknown TTS engine '{engine_name}' in config.toml. "
                "TTS disabled.",
                file=sys.stderr
            )
            return None
        ...
        print(
            f"Error creating TTS engine from configuration: {e}",
            file=sys.stderr
        )
        return None
